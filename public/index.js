...
function userFound() {
    ...
    document.getElementById("welcomeUser").innerHTML = user.email;
}
...
function elementForTeams(doc, id) {
    ...
    span.classList.add("userAdmin");
    span.innerText = "userAdmin"
...
function displayTeam2() {
    ...
    db.collection("users/" + user.email + "/member").get().then((querySnapshot) => {
        var userAdmins = document.getElementsByClassName("userAdmin");
        j = userAdmins.length;
        querySnapshot.forEach((doc) => {
...
function displayMembers() {
    ...
    docRef.get()
        .then((querySnapshot) => {
            querySnapshot.forEach((doc) => {
                userAdmin.push(doc.id);
            })
            var j = -1
            for (var i = 0; i < userAdmin.length; i++) {
                var docRef2 = db.collection("users/" + user.email + "/teams/" + userAdmin[i] + "/members");
                docRef2.get()
                    .then((querySnapshot) => {
                        j++
                        querySnapshot.forEach((doc) => {
